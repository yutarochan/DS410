...
object ReviewPredict {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val ratings = review_df.select("reviewerID", "asin", "overall").rdd.map(x => (x(0).toString, x(1).toString, x(2).toString.toDouble)).partitionBy(new HashPartition(100)).persist(MEMORY_AND_DISK_SER)
